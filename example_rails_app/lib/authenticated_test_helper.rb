module AuthenticatedTestHelper
  ...
  def login_as(user)
    ...
    @request.session[:user] = user ? user.id : nil   if @request
    User.current_user = user ? user : nil
  end
  ...
  #  end
  # 
  def assert_difference(object, method = nil, difference = 1, message = "\#{object}#\#{method}")
    initial_value = object.send(method)
    yield
    assert_equal initial_value + difference, object.send(method), eval(message)
  end
